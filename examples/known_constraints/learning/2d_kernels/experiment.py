...
ROI_DESCRIPTION = ROIDescription(
    jnp.array([[[-1, 0], [-1.5, -0.5]], [[-2, -1], [-1.5, 0.5]]])
)
...
def experiment(
    seed: int, alg: str | None, kernel_name: str, lengthscale: float, name: str
):
    wandb.init(
        name="known_constraints/learning/2d_kernels",
    ...
    jax_has_gpu()
    print(
        "SEED:", seed, "ALG:", alg, "KERNEL:", kernel_name, "LENGTHSCALE:", lengthscale
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name", type=str, default="known_constraints/learning/2d_kernels"
    )
